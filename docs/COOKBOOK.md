...
| Auth/content errors waste retry budget | Permanent failure detection skips retry immediately | `ErrorClassifier` + `FailureKind.AUTH_ERROR` |
| Process crashes lose conversation state | Event-sourced sessions with crash recovery | `FileStore` + `harness.resume()` |
| Rate limits cause cascading retries | Respects provider Retry-After headers | `retry_after_seconds` in classification |
