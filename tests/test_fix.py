...
async def test_fixes_failing_suite_and_verifies(tmp_path):
    ...
    result = await fix_tests(proj, model=model, test_command="pytest -q", max_steps=6)
...
async def test_already_green_is_a_noop(tmp_path):
    ...
    result = await fix_tests(proj, model=model, test_command="pytest -q")
...
async def test_reports_unfixed_when_agent_fails(tmp_path):
    ...
    result = await fix_tests(proj, model=model, test_command="pytest -q", max_steps=3)
